...
@echo constexpr char GBEMU_VERSION[] = "%version%"; > %0\..\..\src\version.h 
...
copy %resources%\* %build%  && ssed -i".bak" "s/@GBEmuVersion@/%version%/g" %build%\res.rc && rc -fo  %build%\res.res %build%\res.rc && %CC% %CPPFLAGS% %0\..\..\src\sdl_main.cpp -TP -c -o%build%\sdl_main.obj && %CC% %build%\sdl_main.obj %build%\res.res %build%\imgui.obj -Zi %0\..\lib\SDL2Main.lib Opengl32.lib Shell32.lib ComDlg32.lib %0\..\lib\SDL2.lib -o%build%\gbemu.exe -MT -link -SUBSYSTEM:WINDOWS
