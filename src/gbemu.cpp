...
void writeByte(u8 byte, u16 address, MMU *mmu, GameBoyDebug *gbDebug) {
    ...
    case 0x8000 ... 0x9FFF: { 
        ...
        if (!lcd->isEnabled || lcd->mode != LCDMode::ScanVRAMAndOAM) {
            ...
            gbDebug->hitBreakpoint = gbDebug->lostVRAMWriteBP;
        }
    ...
    case 0xFE00 ... 0xFE9F: {
        if (!lcd->isEnabled || mmu->isDMAOccurring || (lcd->mode != LCDMode::ScanVRAMAndOAM && lcd->mode != LCDMode::ScanOAM)) {
            ...
            gbDebug->hitBreakpoint = gbDebug->lostOAMWriteBP;
        }
    } break;
...
//        if (mmu->frameStep != 0 && mmu->frameStep != 2 &&
//                mmu->frameStep != 4 && mmu->frameStep != 6 && sq1->isLengthCounterEnabled &&
//                !wasEnabled && sq1->lengthCounter != 0) {
//            sq1->lengthCounter--;
//            if (sq1->lengthCounter == 0) {
//                sq1->isEnabled = false;
//            }
...
static void executeCurrentInstruction(CPU *cpu, MMU *mmu, GameBoyDebug *gbDebug) {
    ...
    case 0xCB: { //CB prefixes
        ...
        case 4: src = cpu->H; break;
        case 5: src = cpu->L; break;
        case 6: src = readByte(word(cpu->H, cpu->L), mmu); break;
        case 7: src = cpu->A; break;
        default: cpu->didHitIllegalOpcode = true; return;
        }
...
static bool willBranch(CPU *cpu) {
    ...
    case 0x20: 
       return IS_FLAG_CLEAR(Z); 
    case 0x28: 
       return IS_FLAG_SET(Z); 
    case 0x30: 
       return IS_FLAG_CLEAR(C); 
    case 0x38: 
       return IS_FLAG_SET(C); 
    case 0xC0: 
       return IS_FLAG_CLEAR(Z); 
    case 0xC2: 
       return IS_FLAG_CLEAR(Z); 
...
static void loadNextInstruction(CPU *cpu, MMU *mmu) {
...
static void stepLCD(LCD *lcd, u8 *outRequestedInterrupts, GameBoyDebug *gbDebug) {
...
static void stepSweep(MMU::SquareWave1 *sq1) {
    sq1->sweepClock++;
...
static void stepEnvelope(MMU::SquareWave1 *sq1, MMU::SquareWave2 *sq2, MMU::Noise *noise) {
...
static void stepLength(MMU::SquareWave1 *sq1, MMU::SquareWave2 *sq2, MMU::Wave *wave, MMU::Noise *noise) {
...
void machineStep(CPU *cpu, MMU* mmu, GameBoyDebug *gbDebug, int volume) {
    ...
    mmu->cyclesSinceLastFrameSequencer += CYCLES_PER_STEP;
    
    if (mmu->cyclesSinceLastFrameSequencer >= FRAME_SEQUENCER_PERIOD) {
        CO_ASSERT(mmu->cyclesSinceLastFrameSequencer == FRAME_SEQUENCER_PERIOD);
        switch (mmu->soundFrameStep) {
        case 0:
        case 4: {
            stepLength(sq1, sq2, wave, noise);
        } break;
        case 2:
        case 6: {
            stepLength(sq1, sq2, wave, noise);
        ...
        case 7: {
            stepEnvelope(sq1, sq2, noise);
        } break; 
            
        }
        mmu->soundFrameStep++;
        if (mmu->soundFrameStep > 7) {
            mmu->soundFrameStep = 0;
        }
        mmu->cyclesSinceLastFrameSequencer = 0;
    }
    
    
    //duty
    sq1->frequencyClock += CYCLES_PER_STEP;
    ...
    sq2->frequencyClock += CYCLES_PER_STEP;
    ...
    wave->frequencyClock += CYCLES_PER_STEP;
    ...
    noise->frequencyClock += CYCLES_PER_STEP;
    ...
    mmu->cyclesSinceLastSoundSample += CYCLES_PER_STEP;
    ...
    profileEnd(profileState);
    stepLCD(&mmu->lcd, &tmpRequestedInterrupts, gbDebug);

    //DMA
    if (mmu->isDMAOccurring) {
#define DMA_CYCLES_PER_BYTE 4
        mmu->cyclesSinceLastDMACopy += CYCLES_PER_STEP;
...
#define DIVIDER_CYCLES_PER_INCREMENT 256
        mmu->cyclesSinceDividerIncrement += CYCLES_PER_STEP;
        ...
        if (mmu->isTimerEnabled) {
            mmu->cyclesSinceTimerIncrement += CYCLES_PER_STEP;
